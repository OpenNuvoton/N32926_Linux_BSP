                                ...
                                struct snd_pcm_hw_params *params)
{
	...
	ENTER();
		...
		DBG("SPU allocate buffer size: %d\n",params_buffer_bytes(params));
...
#if defined(CONFIG_SND_SOC_W55FA92_I2S) || defined(CONFIG_SND_SOC_W55FA92_I2S_MODULE)
        struct snd_pcm_runtime *runtime = substream->runtime;
        struct w55fa92_audio *w55fa92_audio = runtime->private_data;
...
static void report_addr(unsigned long data)
{
    ...
    w55fa92_audio = runtime->private_data;
...
#ifdef OPT_SOLVE_LAST_PERIOD_REPEATED
static int w55fa92_dma_trigger(struct snd_pcm_substream *substream,
                              int cmd)
{
        struct w55fa92_audio *w55fa92_audio = substream->runtime->private_data;
        int ret = 0;	
		spin_lock(&w55fa92_audio->lock);
