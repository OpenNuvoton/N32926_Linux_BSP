...
/*  
	Create by SWChou
	       ...
	       | 8 bit I2C slave addr | 7 bit I2C slave addr
	------------------------------------------------------
	XC7021 |        0x36          |       0x1B 	
	------------------------------------------------------
    SC2133 |        0x60          |       0x30         
...
static void NTSetResolution(int index)
{
	...
	uSensorID = g_uSensorID;	//Hardware mode useless	
...
static void XC7021RegConfig(void)
{
	...
	if(1){
		...
		printk("detected ISP id0=%0x id1=%02x\n",id0,id1);
	}
}
...
static __s32 InitSensor(__u32 u32Sensor, void *priv)
{
	...
	printk("Init xc7021 in port 1\n");
	vin_priv->pDevVin->Init(TRUE, eVIDEOIN_SNR_UPLL, 10000, eVIDEOIN_SNR_CCIR601);	//Frank modify 12000
	...
	{/* If exist the second video capture device ==> Enable 2nd port's pin function  */
		if( (inp32(REG_AHBCLK2) & VIN1_CKE) == 0)/* VideoIn port 2 close */{
			...
			video_port2_pin_init(TRUE, eVIDEOIN_SNR_UPLL, 10000, eVIDEOIN_2ND_SNR_CCIR601);	
			printk("Init port 2 for reset and power down\n");
	#endif																
	#ifdef CONFIG_VIDEO_PORT2_1_DEV2	
			video_port2_pin_init(TRUE, eVIDEOIN_SNR_UPLL, 10000, eVIDEOIN_3RD_SNR_CCIR601);
...
#else
	printk("-----Frank will powerdown and reset sensor\n");
	Snr1_PowerDown(FALSE);
	Snr1_Reset(TRUE);	
#endif	
