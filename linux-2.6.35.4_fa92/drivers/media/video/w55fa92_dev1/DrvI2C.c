...
#if 0
	/* Multiplt pin function */
	if(u32SCKPinMask<0x100)
		outp32( (REG_GPAFUN0+ (u32SCKPortIndex>>4)*8), (inp32( (REG_GPAFUN0+ (u32SCKPortIndex>>4)*8))& ~(3<<(u32SckBit *2))) );
	else
		outp32( (REG_GPAFUN1+ (u32SCKPortIndex>>4)*8), (inp32( (REG_GPAFUN1+ (u32SCKPortIndex>>4)*8))& ~(3<<(u32SckBit *2))) );

	if(u32SDAPinMask<0x100)
		outp32( (REG_GPAFUN0+ (u32SDAPortIndex>>4)*8), (inp32( (REG_GPAFUN0+ (u32SDAPortIndex>>4)*8))& ~(3<<(u32SdaBit *2))) );
	else	
		outp32( (REG_GPAFUN1+ (u32SCKPortIndex>>4)*8), (inp32( (REG_GPAFUN1+ (u32SCKPortIndex>>4)*8))& ~(3<<(u32SckBit *2))) );
#endif 
