...
static __s32 InitSensor(__u32 u32Sensor, void *priv)
{
	...
	printk("Init OV10633 in port 1\n");
	vin_priv->pDevVin->Init(TRUE, eVIDEOIN_SNR_UPLL, 19000, eVIDEOIN_SNR_CCIR601);	
	...
	{/* If exist the second video capture device ==> Enable 2nd port's pin function  */
		if( (inp32(REG_AHBCLK2) & VIN1_CKE) == 0)/* VideoIn port 2 close */{
			...
			video_port2_pin_init(TRUE, eVIDEOIN_SNR_UPLL, 19000, eVIDEOIN_2ND_SNR_CCIR601);	
			printk("Init port 2 for reset and power down\n");
	#endif																
	#ifdef CONFIG_VIDEO_PORT2_1_DEV2	
			video_port2_pin_init(TRUE, eVIDEOIN_SNR_UPLL, 19000, eVIDEOIN_3RD_SNR_CCIR601);
