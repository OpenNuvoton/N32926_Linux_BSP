...
)
{
	...
	if(bIsRead)
		*pi32Value = (DrvVideoIn_I2cReadOV(vin_priv->sensor_intf->u8SensorDevID, TW9900_BRIGHT_CTL)&0xff);
	else
		DrvVideoIn_I2cWriteOV(vin_priv->sensor_intf->u8SensorDevID, TW9900_BRIGHT_CTL, *pi32Value);
...
)
{
	...
	if(vin_priv->sensor_intf->u8SensorDevID != g_uOvDeviceID[0])
		return FALSE;
	if(bIsRead)
		*pi32Value = (DrvVideoIn_I2cReadOV(vin_priv->sensor_intf->u8SensorDevID, TW9900_CONTRAST_CTL)&0xff);
	else
		DrvVideoIn_I2cWriteOV(vin_priv->sensor_intf->u8SensorDevID, TW9900_CONTRAST_CTL, *pi32Value);
...
)
{
	...
	if(vin_priv->sensor_intf->u8SensorDevID != g_uOvDeviceID[0])
		return FALSE;
	if(bIsRead)
		*pi32Value = (DrvVideoIn_I2cReadOV(vin_priv->sensor_intf->u8SensorDevID, TW9900_SATURATION_CTL)&0xff);
	else{
		DrvVideoIn_I2cWriteOV(vin_priv->sensor_intf->u8SensorDevID, TW9900_SATURATION_CTL, *pi32Value);		//U Saturation
		DrvVideoIn_I2cWriteOV(vin_priv->sensor_intf->u8SensorDevID, TW9900_SATURATION_CTL+1, *pi32Value);		//V Saturation
	}
	return TRUE;
}
#if 0
