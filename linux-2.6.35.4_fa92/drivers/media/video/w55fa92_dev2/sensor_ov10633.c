...
#define OV10633_CHIP_ID_H	0x7F
#define OV10633_CHIP_ID_L	0xA2
...
static struct NT_RegValue g_sOV10633_Init[] = 
{//24MHz sensor clock
	#include "OV10633/OV10633_Init.dat" 
};
static struct NT_RegValue g_sOV10633_HD720[] = 
{//20MHz sensor clock ==> 25f/s   24:80=20:x ==> x = 66.666~MHz
	#include "OV10633/OV10633_HD720.dat"
};

static struct NT_RegValue g_sOV10633_VGA[] = 
{//20MHz sensor clock
  	#include "OV10633/OV10633_VGA.dat"
};
static struct NT_RegTable g_OV10633_InitTable[] =
{
...
static __u8 g_uOvDeviceID= 0x60;	// OV10633
...
static int s_i32SensorClk = 19000;
static void NTSetResolution(int index)
{
	...
	uTableSize = g_OV10633_InitTable[0].uTableSize;
	...
	uTableSize = g_OV10633_InitTable[index].uTableSize;
	psRegValue = g_OV10633_InitTable[index].sRegTable;
	printk("Programming sensor resolution = %d\n", index);
	for(i=0;i<uTableSize; i++, psRegValue++){
		udelay(10);
		DrvVideoIn_I2cWriteNT(uDeviceID, (psRegValue->uRegAddr), (psRegValue->uValue));
	}
...
#ifndef CONFIG_SHARE_SENSOR
static void OV10633RegConfig(void)
{
  	__u32 i;
	__u16 uTableSize;
	__u8  uDeviceID;
	...
	uTableSize = g_OV10633_InitTable[REG_VALUE_INIT].uTableSize;
	psRegValue = g_OV10633_InitTable[REG_VALUE_INIT].sRegTable;
	uDeviceID = g_uOvDeviceID;
...
static __s32 InitSensor(__u32 u32Sensor, void *priv)
{
	...
	printk("Init OV_10633 \n");
#ifdef CONFIG_VIDEO_PORT2_0_DEV2
	printk("Init port 2 \n");
	vin_priv->pDevVin->Init(TRUE, eVIDEOIN_SNR_UPLL, 19000, eVIDEOIN_2ND_SNR_CCIR601);	
#endif															
#ifdef CONFIG_VIDEO_PORT2_1_DEV2
	printk("Init port 2_1 \n");	
	vin_priv->pDevVin->Init(TRUE, eVIDEOIN_SNR_UPLL, 19000, eVIDEOIN_3RD_SNR_CCIR601);
#endif
	...
	{/* If exist the first video capture device  */		
		if( (inp32(REG_AHBCLK) & VIN0_CKE) == 0)/* If VideoIn 1 close, do reset */{
			/* 2014-01-10 */	
			video_port1_pin_init(TRUE, eVIDEOIN_SNR_UPLL, 19000, eVIDEOIN_SNR_CCIR601);		
#ifndef CONFIG_SHARE_SENSOR
	...
	vin_priv->pDevVin->Open(96000, 19000);
	...
	vin_priv->pDevVin->SetSensorPolarity(TRUE, FALSE, FALSE);
...
)
{
...
#ifndef CONFIG_SHARE_SENSOR
	#ifdef CONFIG_VIDEO_PORT2_0_DEV2
	if( i8SensorIdx == REG_VALUE_VGA )
		s_i32SensorClk = 24000;
	else{		 
		if(s_i32SensorClk>19000)
			s_i32SensorClk = 19000;	
	}
	vin_priv->pDevVin->Init(TRUE, eVIDEOIN_SNR_UPLL, s_i32SensorClk, eVIDEOIN_2ND_SNR_CCIR601);
	#endif
	...
	vin_priv->pDevVin->Init(TRUE, eVIDEOIN_SNR_UPLL, s_i32SensorClk, eVIDEOIN_3RD_SNR_CCIR601);	
	#endif
...
{
	*sensor_intf = (NVT_SENSOR_T*)(&nvt_sensor_ov10633);
	return 0;
}
#endif 
