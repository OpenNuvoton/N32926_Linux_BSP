...
#ifndef CONFIG_SHARE_SENSOR
static u8 GM7150SetVideoformat(int std)
{
	u8  uDeviceID;
	u8 	fmt = 0;
	u8  ret;	
	...
	printk("GM7150 Video format : %02x. \r\n",fmt);
	ret = DrvVideoIn_I2cWriteOV(uDeviceID, GM7150_VIDEO_STD, fmt);
	if(ret==FALSE){
...
static u8 GM7150SetInputSource(int src)
{
	u8  uDeviceID;
	u8  inputsrc = 0;
	u8  ret;
	...
	if (src == GM7150_A1P1A) {
		...
		printk("GM7150 Video Input Source : %02x. GM7150 Channel = A1P1A mode. \r\n",inputsrc);
	} else if (src == GM7150_A1P1B) {
		inputsrc=0x02;	/* A1P1B mode */
		printk("GM7150 Video Input Source : %02x. GM7150 Channel = A1P1B mode. \r\n",inputsrc);
	} else {
		inputsrc=0x01;	/* S-Video mode */
		printk("GM7150 Video Input Source : %02x. GM7150 Channel = SVIDEO mode. \r\n",inputsrc);
	}

	ret = DrvVideoIn_I2cWriteOV(uDeviceID, GM7150_VD_IN_SRC_SEL_1, inputsrc);
	return ret;
}
#endif
...
#define GM7150_BRIGHT_CTL_DEV2			0x09
#define GM7150_CONTRAST_CTL_DEV2		0x0C
#define GM7150_SATURATION_CTL_DEV2		0x0A
static BOOL	
...
)
{
	...
	if(vin_priv->sensor_intf->u8SensorDevID != g_uOvDeviceID[0])
		return FALSE;
	if(bIsRead)
		*pi32Value = (DrvVideoIn_I2cReadOV(vin_priv->sensor_intf->u8SensorDevID, GM7150_BRIGHT_CTL_DEV2)&0xff);//GM7150
	else
		DrvVideoIn_I2cWriteOV(vin_priv->sensor_intf->u8SensorDevID, GM7150_BRIGHT_CTL_DEV2, *pi32Value);
...
)
{
	...
	if(vin_priv->sensor_intf->u8SensorDevID != g_uOvDeviceID[0])
		return FALSE;
	if(bIsRead)
		*pi32Value = (DrvVideoIn_I2cReadOV(vin_priv->sensor_intf->u8SensorDevID, GM7150_CONTRAST_CTL_DEV2)&0xff);//GM7150
	else
		DrvVideoIn_I2cWriteOV(vin_priv->sensor_intf->u8SensorDevID, GM7150_CONTRAST_CTL_DEV2, *pi32Value);
...
)
{
	...
	if(vin_priv->sensor_intf->u8SensorDevID != g_uOvDeviceID[0])
		return FALSE;
	if(bIsRead)
		*pi32Value = (DrvVideoIn_I2cReadOV(vin_priv->sensor_intf->u8SensorDevID, GM7150_SATURATION_CTL_DEV2)&0xff);//GM7150
	else
		DrvVideoIn_I2cWriteOV(vin_priv->sensor_intf->u8SensorDevID, GM7150_SATURATION_CTL_DEV2, *pi32Value);
