...
u32 volatile u32V4l2GadgetUserBufferUsed = 0;
u32 volatile u32V4l2GadgetUserBufferPAddr = 0;
u32 volatile u32V4l2GadgetUserBufferVAddr = 0;
u32 volatile u32V4l2GadgetUserBufferNum = 0;
u32 volatile u32V4l2GadgetUserBufferOffset = 0;
...
static int uvc_free_buffers(struct uvc_video_queue *queue)
{
	...
	if (queue->count) {
		...
		if(queue->mem)
		{
			if(u32V4l2GadgetUserBufferUsed == 0)
			{
		  ...
		  unsigned int buflength)
{
	...
	for (; nbuffers > 0; --nbuffers) {
		if(u32V4l2GadgetUserBufferUsed == 0)
			...
			mem = phys_to_virt(u32V4l2GadgetUserBufferPAddr); 	
	...
	for (i = 0; i < nbuffers; ++i) {
		...
		{
			queue->buffer[i].buf.m.offset = i * u32V4l2GadgetUserBufferOffset;
			queue->buffer[i].buf.length = u32V4l2GadgetUserBufferOffset;	
		}
...
static int uvc_queue_waiton(struct uvc_buffer *buf, int nonblocking)
{
	...
	{
		u32 data = (__raw_readl(REG_USBD_EPA_DATA_CNT)>>(16-4));

		if(__raw_readl(REG_USBD_DMA_CTRL_STS)&0x20)
		{
			if(__raw_readl(REG_USBD_DMA_CNT) == 0xF0000)
					return -1;     
			else if(__raw_readl(REG_USBD_DMA_CNT) == 0x96000)
			{
				if(data > ( __raw_readl(REG_USBD_DMA_CNT) /4))
					return -1;       
				else
					return 0;
			}
			...
			{
				if(data > ( __raw_readl(REG_USBD_DMA_CNT) /8))
					return -1;        
				else
					return 0;
			}			
...
uvc_queue_mmap(struct uvc_video_queue *queue, struct vm_area_struct *vma)
{
	...
	if(u32V4l2GadgetUserBufferUsed == 1)
		ret = -EINVAL;
