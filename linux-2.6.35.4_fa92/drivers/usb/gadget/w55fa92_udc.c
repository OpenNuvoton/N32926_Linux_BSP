...
int volatile usb_ep_status[W55FA92_ENDPOINTS] = {0};
...
static int w55fa92_pullup(struct usb_gadget *_gadget, int is_on)
{
	struct w55fa92_udc  *dev;
	unsigned long   flags;
	...
	dev = container_of (_gadget, struct w55fa92_udc, gadget);
	...
	int volatile i=0,index =0;
        ...
        case USBR_SET_FEATURE:
			...
			if((pcrq->bRequestType & 0x3) == 0x0) 	/* Receipent is Device */
			{
				...
				{
					 __raw_writel(pcrq->wIndex >> 8, REG_USBD_TEST);		
							 printk("TEST PACKET\n");
				}
			}
                for (i = 1; i < W55FA92_ENDPOINTS; i++) {
...
static int /*__init*/ w55fa92_udc_probe(struct platform_device *pdev)
{
	...
	printk("w55fa92_udc_probe 20180820\n");
        udc->pdev = pdev;
