...
static struct w55fa92_nand_ecclayout w55fa92_nand_EXECUTE_oob;
...
/*****************************************/
	 ...
	 .ecclayout = (struct nand_ecclayout*)&w55fa92_nand_EXECUTE_oob
        },
/******************************************/
        {
         .name = "NAND1-1",
...
static int __devinit w55fa92_nand_probe(struct platform_device *pdev)
{
	...
	*/
...

	{
		...
		i32eBCHAlgo = w55fa92_hwecc_choice_bch_algorithm ( ePageSize,   mtd->oobsize, 	0 );
		w55fa92_nand_EXECUTE_oob.m_BCHAlgoidx = i32eBCHAlgo;

		if ( ePageSize==3 && mtd->oobsize >= g_SYSAREA_NAND_EXTRA_SIZE[ePageSize] ) // 8K, oob >= 376Byte
		{
			w55fa92_nand_EXECUTE_oob.m_SMRASize	= g_SYSAREA_NAND_EXTRA_SIZE[ePageSize];
			mtd->oobsize 				= w55fa92_nand_EXECUTE_oob.m_SMRASize;
		}
		else
			w55fa92_nand_EXECUTE_oob.m_SMRASize	= mtd->oobsize;
