...
static void w55fa92_irq_gpio_ack(unsigned int irq)
{
	__raw_writel(0x01, REG_AIC_EOSCR);
}

static void w55fa92_irq_gpio_mask(unsigned int irq)
{
...
static void w55fa92_irq_gpio_unmask(unsigned int irq)
{
...
static int w55fa92_irq_gpio_set_type(unsigned int irq, unsigned int type)
{
	unsigned int port,num;
...
static struct irq_chip w55fa92_irq_gpio_chip = {
	.name		= "GPIO-IRQ",
	.ack		= w55fa92_irq_gpio_ack,
	.mask		= w55fa92_irq_gpio_mask,
	.unmask		= w55fa92_irq_gpio_unmask,
	.set_type	= w55fa92_irq_gpio_set_type,
};

static irqreturn_t w55fa92_gpio_irq(int irq, void *dev_id) 
{
	...
	for(i=0; i<GPIO_PORT_NUM; i++)
	{
		...
		if((isr != 0) && (tmp != 0))
		{
			//printk("w55fa92_gpio_irq!0x%x 0x%x\n",irq ,isr);
...
static int __init w55fa92_init_irq_gpio(void)
{
	int irqno;

        if (request_irq(IRQ_GPIO3, w55fa92_gpio_irq, IRQF_DISABLED, "gpio",NULL) != 0) {
                printk("register the gpio_irq failed!\n");
                return -1;
        }	

	for (irqno = IRQ_GPIO_START; irqno < IRQ_GPIO_END; irqno++) {
		set_irq_chip(irqno, &w55fa92_irq_gpio_chip);
