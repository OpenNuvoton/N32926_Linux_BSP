/*
 * linux/arch/arm/mach-w55fa92/clock.c
 *
...
void w55fa92_ahbclk2_enable(struct clk *clk, int enable)
{
	unsigned int clocks = clk->cken;
	unsigned long clken;

	clken = __raw_readl(REG_AHBCLK2);
	//printk("\tREG_AHBCLK2=0x%x, clocks=0x%x\n", readl(REG_AHBCLK2), clocks);
	...
	__raw_writel(clken, REG_AHBCLK2);
	//printk("\tREG_AHBCLK2=0x%x\n", readl(REG_AHBCLK2));
}

void w55fa92_apbclk_enable(struct clk *clk, int enable)
{
	unsigned int clocks = clk->cken;
	unsigned long clken;
